...
public class UserProvider 
{
	...
	{
		Provider oldProvider = providerService.getProviderByFirstNameAndLastNameAndPickName(provider.getFirstName(), provider.getLastName(), provider.getPickName());
		if (oldProvider != null && oldProvider.getId() != provider.getId())
		{
			errors.add("Another provider with same first name and last name already exists");
		}
		
		oldProvider = providerService.getProviderByPhoneNumber(provider.getPhoneNumber());
		if (oldProvider != null && oldProvider.getId() != provider.getId())
		{
